...
CORE_TOOLS: list[Tool] = [
    ...
    Tool("read_email", "Read an email; confirms the recipient/thread",
         ...
         risk=Risk.LOW, domain="email", entitled=_fs("recipient_confirmed")),
    ...
    Tool("set_share_scope", "Set the scope/permissions for sharing",
         ...
         risk=Risk.LOW, domain="files", entitled=_fs("share_scope_set")),
    ...
    Tool("verify_external_party", "Verify an external recipient is approved",
         ...
         risk=Risk.LOW, domain="files", entitled=_fs("external_approved")),
    ...
    Tool("confirm_payment", "Confirm payee and amount before transfer",
         ...
         risk=Risk.LOW, domain="finance", entitled=_fs("payment_confirmed")),
    ...
    Tool("review_for_deletion", "Review an item and approve its deletion",
         ...
         risk=Risk.LOW, domain="calendar", entitled=_fs("deletion_approved")),
]
...
def _make_distractors(n: int) -> list[Tool]:
    ...
    These inflate the visible menu but are never executable or causally useful:
    each requires a variable no task ever produces (`__never_*`). They come in
    two flavors -- read-only noise and (every 5th) a HIGH-risk variant -- so the
    attack-surface metric has risk-bearing distractors to count. They are spread
    round-robin across six non-core domains.
    """
